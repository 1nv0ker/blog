---
name: sanity-content-blog-en-publish
description: "Draft, validate, preview, probe, and publish one bilingual Sanity blog-en document through the bundled sanityblog MCP server. Use only when the user explicitly requests a final remote publish and accepts one external write."
---

# Sanity Blog EN Publish

Publish one blog-en document while preserving the accepted local bundle and safe result as the source of truth.
...
- `sanity_content_probe_publish({contentType: "blog-en", articlePath, previewRevision})`
...
- `sanity_content_publish({contentType: "blog-en", articlePath, previewRevision})`
...
4. Call `sanity_content_prepare_publish({contentType: "blog-en", baseSlug})` once and capture all returned paths and identifiers.
5. Write only the returned staging bundle, then call `sanity_content_validate({contentType: "blog-en", articlePath})` until valid.
6. Call `sanity_content_preview({contentType: "blog-en", articlePath})`. Repeat validation and preview after every edit until the user accepts the current `previewRevision`.
7. Call `sanity_content_probe_publish({contentType: "blog-en", articlePath, previewRevision})` exactly once with that revision. Continue only for explicit `mode: "create"` or sanitized conflict-driven `mode: "update"`.
...
10. If declined, call `sanity_content_release({contentType: "blog-en", slug, reservationId})` only when safe.
11. If confirmed, call `sanity_content_commit({contentType: "blog-en", slug, reservationId})` once. If it reports `committed: true` with cleanup failure, use its authoritative paths and never retry commit.
12. Call `sanity_content_preview({contentType: "blog-en", articlePath})` on the authoritative committed path and require the revision to equal the accepted revision.
13. Call `sanity_content_publish({contentType: "blog-en", articlePath, previewRevision})` exactly once. It performs its own revision-bound dry-run and at most one final POST or PUT mutation; do not add another probe or retry.
