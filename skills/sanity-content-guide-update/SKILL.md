---
name: sanity-content-guide-update
description: "Revise, validate, preview, probe, and update an existing bilingual Sanity guide document through the bundled sanityblog MCP server. Use only for an identified existing document and never to create a missing one."
---

# Sanity Guide Update

Update one existing guide document through a revision-guarded PUT-only workflow. A missing local or remote document is a hard stop.

## Allowed tools

- `sanity_content_check_config({})`
- `sanity_content_start_config_setup({})`
- `sanity_content_prepare_update({contentType: "guide", slug})`
- `sanity_content_validate({contentType: "guide", articlePath})`
- `sanity_content_preview({contentType: "guide", articlePath})`
- `sanity_content_probe_update({contentType: "guide", articlePath, previewRevision})`
- `sanity_content_commit({contentType: "guide", slug, reservationId})`
- `sanity_content_release({contentType: "guide", slug, reservationId})`
- `sanity_content_update({contentType: "guide", articlePath, previewRevision})`
...
3. Call `sanity_content_prepare_update({contentType: "guide", slug})`. Capture every returned path and identifier; stop if the authoritative local bundle is missing or incomplete.
...
6. Call `sanity_content_validate({contentType: "guide", articlePath})` until valid, then call `sanity_content_preview({contentType: "guide", articlePath})` and inspect the local preview.
7. Repeat validation and preview after every edit. Continue only after the user accepts the current `previewRevision`.
8. Call `sanity_content_probe_update({contentType: "guide", articlePath, previewRevision})` exactly once. Require `mode: "update"`, the expected ID, and revision; stop on 404 or any ambiguous state.
...
11. If declined, call `sanity_content_release({contentType: "guide", slug, reservationId})` only when safe.
12. If confirmed, call `sanity_content_commit({contentType: "guide", slug, reservationId})` once. If it reports `committed: true` with cleanup failure, use its authoritative paths and never retry commit.
13. Call `sanity_content_preview({contentType: "guide", articlePath})` on the authoritative committed path and require the revision to equal the accepted revision.
14. Call `sanity_content_update({contentType: "guide", articlePath, previewRevision})` exactly once. It performs a fresh PUT dry-run and one guarded PUT; it never creates.
